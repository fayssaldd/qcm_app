...
import { PROF_EXAM_ROUTE, STUDENT_EXAM_ROUTE } from '../../router'
import DropDownMenu from './DropDownMenu'
import { Brain } from 'lucide-react'

export default function NavBarProfs() {
    ...
    <>
        <nav className="flex backdrop-blur-sm fixed w-full top-0 items-center shadow dark:border-b z-40 justify-between flex-wrap p-6">
            ...
            <div className="block lg:hidden">
                <button onClick={()=>setIsShow(!isShow)} className="flex items-center px-3 py-2 border rounded text-primary border-primary hover:text-teal-700 hover:border-teal-700">
            ...
            <div className={`w-full ${!isShow && 'hidden'}  block flex-grow lg:flex lg:items-center lg:w-auto`}>
                <div className="text-sm flex  lg:flex-grow max-md:flex-col max-md:items-start max-md:border-t max-md:mt-4 max-md:pt-4 items-center justify-end ">
                    {/* <NavLink end to={'/profs'} 
                        className={({ isActive }) =>
                            `${isActive && "bg-primary !text-white" } max-md:mb-4 block lg:inline-block lg:mt-0 text-primary hover:bg-primary p-2 rounded hover:text-white mr-4`
                        }
                    >
                        Home
                    </NavLink> */}
                    <NavLink to={PROF_EXAM_ROUTE} 
                        className={({ isActive }) =>
                            `${isActive && "bg-primary !text-white" } max-md:mb-4 block lg:inline-block lg:mt-0 text-primary hover:bg-primary p-2 rounded hover:text-white mr-4`
                        }
                    >
